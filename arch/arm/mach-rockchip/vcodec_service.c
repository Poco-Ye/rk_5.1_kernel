...
static void vpu_reset(struct vpu_subdev_data *data)
{
	...
	if (pservice->rst_a && pservice->rst_h) {
		if (rockchip_pmu_ops.set_idle_request) {
			...
			if (ret < 0)
				vpu_err("idle request timeout occur!\n");
		}
		pr_info("reset in\n");
		if (pservice->rst_v)
		...
		if (rockchip_pmu_ops.set_idle_request) {
			ret = rockchip_pmu_ops.set_idle_request(type, false);
			if (ret < 0)
				vpu_err("idle request release timeout occur!\n");
...
/*
* hw timeout auto reset lead to iommu ilegal state, make the iommu detach failed.
* recovering iommu state by reseting vpu.
*/
	if (data->hw_info->hw_id != HEVC_ID  && atomic_read(&pservice->reset_request) &&
					(pservice->irq_status & 0x40000))
		vpu_reset(data);
	else
